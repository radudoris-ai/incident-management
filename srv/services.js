...
async init() {
  ...
  async onCustomerRead(req, next) {
    ...
    const top = parseInt(req._queryOption?.$top) || 50;
  ...
  async init() {
    ...
    this.before("CREATE", "Incidents", (req) => this.changeUrgencyDueToSubject(req.data));
    this.on('READ', 'Customers', (req, next) => this.onCustomerRead(req, next));
   // this.on('READ', 'MyBusinessPartner', (req, next) => this.onCustomerRead(req, next));
    this.on(['CREATE', 'UPDATE'], 'Incidents', (req, next) => this.onCustomerCache(req, next));

    this.on('READ', 'BusinessPartner', (req) => {
      console.log('PAssing control to remote system...')
  ...
  async onCustomerRead(req, next) {
    console.log('>> delegate to S4 service...', req.query);
    const top = parseInt(req._queryOption?.$top) || 10;
    const skip = parseInt(req._queryOption?.$skip) || 0;
  ...
  async onCustomerCache(req, next) {
    ...
    if (newCustomerId && (newCustomerId !== "") && ((req.event == "CREATE") || (req.event == "UPDATE"))) {
        ...
        bp('*'),
          bp.to_BusinessPartnerAddress(address => {
            address.to_EmailAddress(emails => {
              ...
              address.to_PhoneNumber(phoneNumber => {
      ...
      console.log('>> Persist customer!', customer.BusinessPartnerName);

      if (customer) {
        customer.email = customer.to_BusinessPartnerAddress[0]?.to_EmailAddress[0]?.EmailAddress;
        customer.phone = customer.to_BusinessPartnerAddress[0]?.to_PhoneNumber[0]?.PhoneNumber;
        customer.name = customer.BusinessPartnerName;
      //  delete customer.addresses;
      //  delete customer.name;
        await UPSERT.into(Customers).entries(customer);
